...
def topsis(input_file, weights, impacts, result_file):
    try:
        ...
        ideal_positive = (weighted_normalized_df.max(axis=0) * np.array(impacts)) + (weighted_normalized_df.min(axis=0) * (1 - np.array(impacts)))
        ideal_negative = (weighted_normalized_df.max(axis=0) * (1 - np.array(impacts))) + (weighted_normalized_df.min(axis=0) * (np.array(impacts)))
